 ...
 * Plugin Manager
 * @constructor
 * @param {Kernel} kernel - Vx kernel object
 */
 ...
 *
 * For example, if a plugin instance has dependsOn = ['vx-http'], then we
 * need to ensure that the `vx-http` plugin is also loaded.
...
Plugins.prototype.instantiatePlugin = function (p) {
  ...
  vx  = new VxCtx(PluginConstructor.prototype.name, config, this.pluginInstances);
  plugin = new PluginConstructor(vx, config);
